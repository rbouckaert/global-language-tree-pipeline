...
for (ii in 1:nrow(files1)) {
  ...
  for (ww in sort(as.numeric(unique(kf)))) {
    ...
    for (col1 in colsX) {
      ...
      col_raw <- gsub("_log", "", col1, fixed = TRUE)
      col_raw <- paste0(col_raw, "_raw")
      ...
      if (col1 %in% cols_log) {
        trans_vec <- log1p(raw_vec)
      } else {
        trans_vec <- raw_vec
      }

      mu <- mean(trans_vec, na.rm = TRUE)
      sd_val <- sd(trans_vec, na.rm = TRUE)
      if (is.na(sd_val) || sd_val == 0) {
        sd_val <- 1
      }

      x_values <- seq(min(raw_vec, na.rm = TRUE),
                      max(raw_vec, na.rm = TRUE),
                      length.out = 200)
      ...
      predscit <- res1$mean[res1$name == col1] * trans_x +
        res1$mean[res1$name == "(Intercept)"]
      predscit2 <- res1$mean_sig[res1$name == col1] * trans_x +
