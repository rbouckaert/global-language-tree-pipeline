...
save_plot_pair <- function(plot_obj, out_dir, stem, width = 12, height = 11, scale = 1) {
...
build_mcc_tree_variant_plots <- function(prepped) {
  tree <- prepped$tree
  ...
  # Supplementry ------------------
  supp_gap_position <- "top" # Set to "right" for the original orientation.
  supp_open_angle <- 30      # 30 degrees ~= 11:30 to 12:30 when centered at top.
  ...
  offset <- offset #+ pwidth_curr + ring_gap
  ...
  family_pwidth <- ring_pwidth_cont
...
write_mcc_tree_variant_outputs <- function(prepped, built_plots) {
  ...
  save_plot_pair(built_plots$p_main, out_dir, "Fig_MCC_main_topology_ED", width = 11.6, height = 10.6)
  save_plot_pair(built_plots$p_supp, out_dir, "Fig_MCC_supp_annotations", width = 14.0, height = 11.8)
  ...
  source_lines <- c(
    ...
    paste0("Main figure file stem: Fig_MCC_main_topology_ED"),
    paste0("Supplementary figure file stem: Fig_MCC_supp_annotations"),
...
built_plots <- build_mcc_tree_variant_plots(prepped)
